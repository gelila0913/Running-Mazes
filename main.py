...
# Try to import image loading library
try:
...
# Note: east_wall[i][C-1] will represent the right boundary wall
left_wall = [1 for _ in range(R)] # Represents the far-left boundary gaps
...
# Define Entry (Left) and Exit (Right)
...
# Open the walls
left_wall[start_r] = 0        # Opening the Left Edge (Entry)
east_wall[end_r][C-1] = 0     # Opening the Right Edge (Exit)
...
def draw_rat(r, c, texture_id, angle):
    ...
    if texture_id:
        ...
        draw_cell_marker(r, c, (1, 1, 0), 14.0)
...
def add_cycles():
    ...
    for i in range(1, R-1):
        for j in range(1, C-1):
            if random.random() < 0.05: # Add loops to defeat shoulder-to-wall rule
...
def solve_step():
    ...
    if (r, c) == end_node:
        ...
        # Move one step "out" for visual effect
        current_pos = (r, c + 0.5) 
        PHASE = "FINISHED"
        return
    ...
    dirs = [(-1, 0, 90), (1, 0, 270), (0, 1, 0), (0, -1, 180)]
    random.shuffle(dirs)
...
def main():
    ...
    window = glfw.create_window(1000, 800, "Maze: Opened Entrance & Exit", None, None)
    glfw.make_context_current(window)
    
    global mouse_texture_id
    mouse_texture_id = load_texture("rat.png")
    
    gen_visited[start_node[0]][start_node[1]] = 1
    ...
    while not glfw.window_should_close(window):
        ...
        for i in range(R):
            for j in range(C):
                ...
                # Internal/Right walls
                if north_wall[i][j]: draw_line(x, y, x+CELL_SIZE, y)
                if east_wall[i][j]: draw_line(x+CELL_SIZE, y, x+CELL_SIZE, y-CELL_SIZE)
                
                # Boundary Walls
                if i == R-1: draw_line(x, y-CELL_SIZE, x+CELL_SIZE, y-CELL_SIZE) # Bottom
                if j == 0 and left_wall[i]: draw_line(x, y, x, y-CELL_SIZE)      # Left (with opening)

        # Indicators
        draw_cell_marker(start_node[0], start_node[1], (0, 1, 0), 12.0) # Start
        draw_cell_marker(end_node[0], end_node[1], (1, 0.5, 0), 12.0)   # End

        # Solver paths
        for r, c in path_stack: draw_cell_marker(r, c, (1, 0, 0), 6.0)
        for r, c in dead_ends: draw_cell_marker(r, c, (0, 0.4, 0.7), 6.0)
